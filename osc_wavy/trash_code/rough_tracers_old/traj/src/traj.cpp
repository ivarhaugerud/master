...
int main(int argc, char* argv[]){
  ...
  if (!experimental::filesystem::exists(h5file)){
  ...
  while (t <= T){
	...
	dx2_mean += pow(x_rw[irw]-x_mean, 2)/(Nrw-1);  // Sample variance
      ...
      statfile << t << " " << x_mean << " " << dx2_mean << std::endl;
    }
	...
	ux_rw[irw] = U0*ux;
	uy_rw[irw] = U0*uy;
  break; //break out of the max number of tries loop
      }
      else {
	if (t <= 0.005*T){
	n_declined++;
	declinedfile << t << " " << x_rw[irw]+dx_rw << " "
		     << y_rw[irw]+dy_rw << " " << n_declined << std::endl;
      }}}
    }
    t += dt;
    it += 1;
