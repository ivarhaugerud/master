...
Lx = np.array([3.49, 4.488, 6.28, 8.5, 10.47, 15.0, 31.41])# 15.7, 31.41 
...
for i in range(len(kappa)):
	if abs(kappa[i]-0.74) > 0.02: 
		plt.plot(np.sqrt(omega/Dm), D[:, i], "o", color="C"+str(i), label=r"$\kappa = %3.2f$" % kappa[i], markersize=3)
		plt.plot(np.sqrt(omega/Dm), D[:, i], color="C"+str(i), linewidth=1)
		print(U[:, i]/Dm)
plt.xscale("log")
#plt.yscale("log")
