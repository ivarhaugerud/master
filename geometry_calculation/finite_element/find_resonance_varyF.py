	...
	for l in range(len(Ds)):
		...
		for K in range(len(F0s)):
			...
			for O in range(len(kappas)):
				...
				D_parallels[O, I, K, l] = scpi.trapz(np.real(total_D), t)/(2*np.pi/(omega))
				counter += 1
				print(100*counter/maxxi, taus[I], Ds[l], F0s[K], kappas[O], D_parallels[O, I, K, l])
				...
				"""
				#plt.show()
np.save("data/D_para_vary_kappa_D_F0_tau20_nu1000", D_parallels)
plt.show()
