...
for K in range(len(kappas)):
	...
	D_eff_xi[:, np.argmin(abs(new_k - 2))] += (5+kappa*kappa*xi*xi)*B0_deriv*B0_deriv/2
	D_eff_xi[:, np.argmin(abs(new_k + 2))] += (5+kappa*kappa*xi*xi)*np.conj(B0_deriv)*np.conj(B0_deriv)/2
	D_eff_xi[:, np.argmin(abs(new_k - 0))] += (5+kappa*kappa*xi*xi)*np.conj(B0_deriv)*(B0_deriv)/2

	for i in range(len(k)):
		D_eff_xi[:, np.argmin(abs(new_k -k[i]))] += kappa*B_minus[:, i] + kappa*xi*B1_min_deriv[:, i]

		if i != 0:
			D_eff_xi[:, np.argmin(abs(new_k-k[i]))] += 2*B0_deriv*B2_0_deriv[:, i-1] - B0_deriv*(B1_plus_deriv[:, i-1] + kappa*kappa*xi*B_plus[:, i-1])

		if i != int(len(k)-1):
			D_eff_xi[:, np.argmin(abs(new_k-k[i]))] += 2*np.conj(B0_deriv)*B2_0_deriv[:, i+1] - np.conj(B0_deriv)*(B1_plus_deriv[:, i+1] + kappa*kappa*xi*B_plus[:, i+1])
