...
for i in range(len(kappa)):
	for j in range(len(tau)):
		T = Ts[j]
		data = np.loadtxt(base+"Lx" +  str(Lx[i]) +"_tau"+ str(round(tau[j], 3)) +"_eps0.5_nu5.0_D3.0_fzero0.0_fone100.0_res150_dt" + str(round(dt[j], 6)) + "/tdata.dat")
