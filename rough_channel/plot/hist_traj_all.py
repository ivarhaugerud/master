...
for i in range(len(Pe)):
    for j in range(len(b)):
        data[str(i)+str(j)+"t"] = np.load("data_analyzed/occupation_time_RZ_b"+str(b[j])+"_Pe" + str(Pe[i]) + ".npy")
...
plt.ylabel('Normalized occurance density')
plt.yscale("log")
#plt.grid(True)
plt.savefig("figures/all_occupation_histogram_b.png")
plt.show()
...
for i in range(len(Pe)):
    ...
    plt.ylabel('Normalized occupation time histogram', fontsize=8)
    plt.legend(loc="best", fontsize=8)
    plt.yscale("log")
    #plt.xscale("log")
    name = (root+"calculate_slope_Re0_Pe"+str(Pe[i])+".pdf")
...
for i in range(len(Pe)):
    plt.errorbar(b, prop_in_RZ[i,:,0], yerr=prop_in_RZ[i,:,1], fmt="o", markersize=3, label="Pe="+str(Pe[i]), color="C"+str(i))
    plt.plot(b, prop_in_RZ[i,:,0],  linewidth=0.5 , color="C"+str(i))
plt.xlabel(r"Roughness $b$", fontsize=8)
plt.ylabel(r"Proportion of particles in RZ", fontsize=8)
plt.legend(loc="best", fontsize=8)
name = (root+"occupation_prop_Re0.pdf")
plt.savefig(name, bbox_inches="tight")
os.system('pdfcrop %s %s &> /dev/null &'%(name, name))
plt.show()

for i in range(len(Pe)):
    plt.errorbar(b, -1/(slopes[i,:,0]), yerr=(1/slopes[i,:,0])*(slopes[i,:,1]/slopes[i,:,0]), fmt="o", markersize=3, label="Pe="+str(Pe[i]), color="C"+str(i))
    plt.plot(b, -1/(slopes[i,:,0]), linewidth=0.5, markersize=3, color="C"+str(i))

plt.xlabel(r"Roughness $b$", fontsize=8)
plt.ylabel(r"Characteristic RZ occupation time $\tau$", fontsize=8)
plt.legend(loc="best", fontsize=8)
plt.yscale("log")
name = root + "slopes_vs_b_Re0.pdf"
plt.savefig(name, bbox_inches="tight")
os.system('pdfcrop %s %s &> /dev/null &'%(name, name))
plt.show()