...
for i in range(len(Pe)):
    for j in range(len(b)):
        data[str(i)+str(j)+"t"] = np.load("data_analyzed/occupation_time_RZ_b"+str(b[j])+"_Pe" + str(Pe[i]) + "Re31.npy")
...
plt.yscale("log")
#plt.grid(True)
plt.savefig("figures/all_occupation_histogram_b_Re31.png")
plt.show()
...
for i in range(len(Pe)):
    ...
    plt.yscale("log")
    name = (root+"calculate_slope_Re31_Pe"+str(Pe[i])+".pdf")
...
for i in range(len(Pe)):
    ...
    plt.errorbar(b, prop_in_RZ[i,:,0], linewidth=0.5, color="C"+str(i))
plt.xlabel(r"Roughness $b$", fontsize=8)
plt.ylabel(r"Proportion of particles in RZ", fontsize=8)
plt.legend(loc="best", fontsize=8)
name = (root+"occupation_prop_Re31.pdf")
plt.savefig(name, bbox_inches="tight")
os.system('pdfcrop %s %s &> /dev/null &'%(name, name))
plt.show()
...
plt.legend(loc="best", fontsize=8)
plt.yscale("log")
name = root + "slopes_vs_b_Re31.pdf"
plt.savefig(name, bbox_inches="tight")
os.system('pdfcrop %s %s &> /dev/null &'%(name, name))
plt.show()